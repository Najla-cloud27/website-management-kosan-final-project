    ...
    <section class="destination-section section-padding fix">
        <div class="container">
            ...
            <div class="row g-4">
                ...
                <div class="col-xl-4 col-lg-4 col-md-6 wow fadeInUp" data-wow-delay=".{{ ($loop->index % 3) * 0.2 }}s">
                    <div class="destination-card-items">
                        <div class="destination-thumb">
                            @if($room->main_image_url)
                                <img src="{{ asset('storage/' . $room->main_image_url) }}" alt="{{ $room->name }}">
                            @else
                            ...
                            <div class="ratting status-ribbon" style="background: #6c757d !important;">
                                ...
                                <span>{{ strtoupper(str_replace('_', ' ', $room->status)) }}</span>
                            </div>
                            @endif
                        </div>
                        <div class="destination-content">
                            ...
                            <p class="room-desc">
                                {{ Str::limit($room->description ?? 'Kamar nyaman dengan fasilitas lengkap', 80) }}
                            </p>
                            ...
                            <div class="room-facilities">
                                <span><i class="fa-solid fa-expand"></i> {{ $room->size ?? '3x4m' }}</span>
                                @if(is_array($fasilitas) && count($fasilitas) > 0)
                                    @foreach($fasilitas as $item)
                                        <span>
                                            <i class="fa-solid {{ $iconMap[$item] ?? 'fa-check' }}"></i> 
                                            {{ $item }}
                                        </span>
                                    @endforeach
                                @else
                ...
                <ul>
                    ...
                    @foreach ($rooms->getUrlRange(1, $rooms->lastPage()) as $page => $url)
                        @if ($page == $rooms->currentPage())
                            <li class="active"><a class="page-numbers">{{ sprintf('%02d', $page) }}</a></li>
                        @else
                            <li><a class="page-numbers" href="#" wire:click.prevent="gotoPage({{ $page }})">{{ sprintf('%02d', $page) }}</a></li>
                        @endif
                    @endforeach
